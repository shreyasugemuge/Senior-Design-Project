...
count = 1
Unit_Release_Date = '2/8/2013'
print('ID, Unit3 Materials Available Date, Student Submit ASR Date, Dates Difference')
for args in sys.argv[1:]:
    df = pd.read_csv(args)
    Submit_ASR_Date = df[df["Object_Title"] == "Submit ASR Unit #3"]["Date"].values.any()
